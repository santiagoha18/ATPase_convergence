---
title: "Epistasis analysis: duplication issue"
...
**Note 1**: This analysis includes the corrected version of the 'TEG+Q111T' construct.
**Note 2**: Preliminary analysis to address a duplication issue in Xenodon+H122D and Xenodon+H111T constructs. Duplicated samples were removed and kept only one good replicate per construct (for Xenodon+H122D and Xenodon+H111T).
...
*Figure S5.* Western blot analysis of Na,K-ATPase with engineered $\alpha$-subunits produced in this study. This corrected version contains the new western blots for TEG+Q111T. See main text for details.
...
df_effect <- read.csv("../data/mut_effects/Percent_change_shared_derived_state_corrected_duplication.csv",header = T)
...
*Note that the point corresponding to the 111T comparison is lower. Original value: 191.87; corrected value: 19.23*

*Note that some points corresponding to the 122D comparisons also change because Xenodon+H122D was involved in three pairwise comparisons*
CHI-FER => from 150.8296751 (old) to 12.78933 (new)
TEG-FER => from 173.078124 (old) to 9.459115 (new)
LEP-FER => from 158.8698757 (old) to 4.742662 (new)
...
ggplot(df_effect,aes(x=Distance,y=PercentDiff),color=AAState) + 
  ...
  annotate("text",x=65,y=180,label=expression(paste(rho, " = -0.18, P > 0.05"))) + ggtitle("Fig 5A")
```
...
# Plot results
label1a <- "Best model: Model D\n (groups 1-4: 16 sites)"
label1b <- expression(paste("R"^2,"= 0.713"))
p1 <- ggplot(df_models_g80,aes(x=group,y=r2_per_group)) + geom_line(size=1.3) + theme_classic() +
  ...
  annotate("text",x=11,y=0.65,label=label1a) + annotate("text",x=11,y=0.61,label=label1b)

label2a <- "Best model: Model D\n (groups 1-4: 9 sites)"
label2b <- expression(paste("R"^2,"= 0.740"))
p2 <- ggplot(df_models_g99,aes(x=group,y=r2_per_group)) + geom_line(size=1.3) + theme_classic() +
  ...
  annotate("text",x=27,y=0.65,label=label2a) + annotate("text",x=27,y=0.61,label=label2b)
...
ggplot(df_effect,aes(x=DistanceFew,y=PercentDiff),color=AAState) + 
  ...
  theme(
    ...
    legend.text = element_text(size=16),
    legend.title = element_text(size=16)) +
  annotate("text",x=5,y=180,label=expression(paste(rho, " = 0.904, P < 0.01"))) + ggtitle("Fig 5B")
...
cov_g1_g4 <- c("covariate","V553","V289","V311","V993") # representative sites of groups 1-4
matrix_g1.4 <-  matrix_anova_indep_1[,colnames( matrix_anova_indep_1) %in% cov_g1_g4]

# ANOVA fitted for best model (model B, see table above)
mod_group1.4 <- aov(effect ~ covariate + V553 + V289 + V311 + V993, data =  matrix_anova_indep_1) # 16 sites
...
for(i in 1:1000){
  ...
  matrix_g1.4$perm_effects <- perm_effects
  # Do ANOVAS on permutated datasets
  m <- aov(perm_effects ~ ., data = matrix_g1.4)
...
abline(v = get_R2(mod_group1.4) ,col="red")
p <- sum(null_r2>=get_R2(mod_group1.4))/length(null_r2) # pval = 0.003

print(paste("P-value of observing an R^2 >= as observed: ", p))

######

effects <- matrix_anova_indep_2$effect
cov_g1_g4 <- c("covariate","V553","V289","V298","V311") # representative sites of groups 1-4
matrix_g1.4 <-  matrix_anova_indep_2[,colnames( matrix_anova_indep_2) %in% cov_g1_g4]

# ANOVA fitted for best model (model D, see table above)
mod_group1.4 <- aov(effect ~ covariate + V553 + V289 + V298 + V311, data =  matrix_anova_indep_2) # 9 sites
